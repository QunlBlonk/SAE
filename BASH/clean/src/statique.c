...
char * findKey(char * nom_fichierClean, char * nom_fichierCorrompu, char * fichierClefStoquer){
    ...
    char * clef = (char *)malloc(2 * sizeof(char)); char * clefTemp = (char*)malloc(2*sizeof(char));
	...
	clef[ci]='\0';
	if (fichierClefStoquer[0] != '\0'){
		FILE * fichierclef = fopen(fichierClefStoquer, "w");
